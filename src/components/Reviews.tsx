...
function Review({imgSrc, className, ...props}: ReviewProps){
  ...
  const POSSIBLE_ANIMATION_DELAYS = ["0s","0.1s","0.2", "0.3", "0.4", "0.5"]
  const animationDelay = POSSIBLE_ANIMATION_DELAYS[Math.floor(Math.random()* POSSIBLE_ANIMATION_DELAYS.length)]

  return <div className={cn("animate-fade-in rounded-[2.25rem] bg-white p-6 opacity-0 shadow-slate-900/5", className)} style={{ animationDelay }} {...props}>
    <Phone imgSrc = {imgSrc} />
  </div>
}
